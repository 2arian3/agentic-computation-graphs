...
| [10 — Research plan: characterizing agentic application graphs](10-research-plan-agentic-application-graphs.md) | Forward-looking proposal grounded in [`agentic applications survey.pdf`](agentic%20applications%20survey.pdf) + this instrument: a formal AAG representation, the **ACG-Bench** dataset strategy, graph-based prediction models (GNN/graph-transformer), RQs, an end-to-end pipeline, and a feasible 3–6 month scope. |
| [11 — Making the applications more complex (build roadmap)](11-making-applications-more-complex.md) | Concrete build to move QA from one graph shape to a controlled distribution of shapes: **Steps 1–4 implemented** (extended tools `calculator`/`compare`/`verify_claim`/`decompose`, procedural KB + task families, BM25 retrieval, external loader + closed-book control) with live-verified diverse graphs, plus the next steps (variance re-run, memory tools, MAS family, real datasets). |
| [12 — Enriched-benchmark first results (family × backbone)](12-enriched-benchmark-results.md) | First empirical run on the enriched benchmark: **480 runs** (40 tasks × 5 families × 6 reps × {7B BF16, 14B FP8}). Width is finally a variable (fan-out 44% width>1); backbone reshapes graph size/accuracy for the same task (7B builds bigger, more accurate graphs; FP8 fans wider but weaker on tool-composition). |
